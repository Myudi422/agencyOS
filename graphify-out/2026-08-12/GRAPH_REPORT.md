...
## Summary
- 1499 nodes · 3313 edges · 84 communities (62 shown, 22 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 388 edges (avg confidence: 0.5)
...
4. `SocialAccount` - 52 edges
5. `WorkspaceMember` - 42 edges
6. `get_user_workspace()` - 41 edges
7. `useStore` - 41 edges
8. `Workspace` - 39 edges
9. `AccountPlatform` - 29 edges
10. `Client` - 28 edges
...
Cohesion: 0.08
Nodes (23): Settings, get_db(), Database Migration Script — Migrate database schema from Stripe to Midtrans.…, Migration Script: Create KOL Campaign & Deliverable Tracker tables, Database Migration — WA OTP Verification Menambah kolom phone_number &…, get_current_user_from_token(), get_me(), get (+15 more)
...
Cohesion: 0.09
Nodes (50): PlanTier, str, Paket langganan — semua plan unlimited akun sosmed, beda di quota post., Subscription aktif milik satu user., RoleEnum, Setting, SubscriptionPlan, SubscriptionStatus (+42 more)

### Community 9 - "get_agent_logs"
Cohesion: 0.29
...
Cohesion: 0.09
Nodes (41): _ensure_utc(), get_calendar_posts(), _in_range(), _parse_and_ensure_utc(), Any, datetime, get, put (+33 more)
...
### Community 18 - "posts.py"
Cohesion: 0.13
Nodes (36): PostStatus, PostTarget, PostType, approve_public_post_review(), create_media_upload_url(), create_post(), delete_post(), generate_ai_caption() (+28 more)

### Community 19 - "fetchApi"
Cohesion: 0.09
Nodes (28): CampaignDetailPage(), DELIVERABLE_STATUS_COLORS, PAYMENT_STATUS_BADGES, DEFAULT_PLANS, PricingPage(), TIER_ORDER, TIER_UI_META, Window (+20 more)
...
Cohesion: 0.14
Nodes (30): _activate_user_subscription(), CheckoutRequest, create_checkout(), get_frontend_url(), get_subscription(), _get_user_from_auth(), list_plans(), midtrans_notification() (+22 more)
...
Cohesion: 0.13
Nodes (16): isDatePast(), LocalPost, LocalPostStatus, PfPost, PfResult, PLATFORM_COLORS, PLATFORM_ICONS, QueuePage() (+8 more)
...
### Community 49 - "User"
Cohesion: 0.07
Nodes (100): KolCampaign, KolCampaignKol, KolCampaignStatus, KolDeliverable, KolDeliverableStatus, KolDeliverableType, KolPaymentStatus, KolProfile (+92 more)

### Community 50 - "get_clients"
Cohesion: 0.33
...
Cohesion: 0.07
Nodes (77): AccountPlatform, AccountStatus, ActivityLog, Client, SocialAccount, AccountBriefingSchema, bulk_action(), BulkActionRequest (+69 more)

### Community 54 - "useAuthStore"
Cohesion: 0.06
Nodes (38): AdminPage(), TabType, TIER_COLORS, TIER_ICONS, OAuthCallbackHandler(), BillingSuccessContent(), dynamic, jsonLd (+30 more)

### Community 56 - "get_dashboard_overview"
Cohesion: 0.40
Nodes (5): get_dashboard_overview(), get, Session, User, Sub-300ms fast executive dashboard endpoint returning real-time metrics, queue…

### Community 58 - "update_agent"
Cohesion: 0.67
Nodes (3): patch, Update agent configuration. Reschedules if schedule fields change., update_agent()

### Community 63 - "QueueService"
Cohesion: 0.18
Nodes (13): Any, Post, Session, QueueService, Queue Engine menggunakan background tasks FastAPI. Mengelola publish job per…, Creates PublishJob records and executes them immediately., Sinkronisasi hasil publish dari PostForMe untuk satu target. Mengambil data…, Kurangi 1 kredit dari user yang membuat post setelah PostForMe konfirmasi… (+5 more)
...
### Community 68 - "reset_subscriptions.py"
Cohesion: 0.17
Nodes (3): Migration Script: Add ai_brief column to posts table, Migration: Tambah tabel post_publish_results dan kolom baru ke posts. Jalankan:…, Reset Subscriptions Script — Clears all user subscriptions in DB. Run: python…

### Community 71 - "activity.py"
Cohesion: 0.26
Nodes (11): clear_all_activity_logs(), delete_activity_log(), get_activity_logs(), get, Session, Ensures that a workspace maintains at most `max_logs` activity log records.…, Retrieves chronological activity audit logs with server-side pagination.…, Deletes a specific activity log entry. (+3 more)

### Community 75 - "kol-campaigns/page.tsx"
Cohesion: 0.14
Nodes (17): KolCampaignsPage(), formatNumberToRupiahString(), getTerbilangShort(), parseRupiahStringToNumber(), RupiahInput(), RupiahInputProps, CampaignCard(), CampaignCardProps (+9 more)

### Community 79 - "useStore"
Cohesion: 0.09
Nodes (30): AccountsPage(), dynamic, PLATFORMS_CONFIG, PaginatedActivityResponse, ClientsPage(), MediaPage(), OnboardingPage(), AccountBriefingModal() (+22 more)
...
## Knowledge Gaps
- **305 isolated node(s):** `PaginatedActivityResponse`, `PUBLIC_PATHS`, `SidebarProps`, `TIER_ICONS`, `TIER_COLORS` (+300 more)
...
- **Why does `User` connect `User` to `get_user_workspace`, `Session`, `reset_subscriptions.py`, `database.py`, `models.py`, `admin.py`, `get_agent_logs`, `agents.py`, `statistics.py`, `get_clients`, `ActivityLog`, `posts.py`, `update_agent`?**
...
- **Are the 28 inferred relationships involving `WorkspaceMember` (e.g. with `AssignPlanByEmailRequest` and `PlanUpdateRequest`) actually correct?**
  _`WorkspaceMember` has 28 INFERRED edges - model-reasoned connections that need verification._
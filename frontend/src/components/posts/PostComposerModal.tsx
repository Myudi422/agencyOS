...
export default function PostComposerModal() {
  ...
  const [showAiAccountPicker, setShowAiAccountPicker] = useState<boolean>(false);
  ...
  const handleGenerateCaptionFromAi = async (overrideAccountId?: string) => {
    ...
    const selectedAccs = availableAccounts.filter(a => selectedAccountIds.includes(a.id));
    if (!overrideAccountId && selectedAccs.length > 1 && !showAiAccountPicker) {
      setShowAiAccountPicker(true);
      return;
    }

    setShowAiAccountPicker(false);
    setIsGeneratingAiCaption(true);

    const targetAccountIds = overrideAccountId && overrideAccountId !== "all"
      ? [overrideAccountId]
      : selectedAccountIds;
    ...
    try {
      ...
      if (res?.caption) {
        ...
        const targetAcc = availableAccounts.find(a => a.id === overrideAccountId);
        toast.success(
          targetAcc
      ...
      <div className="fixed inset-0 w-screen h-screen z-[200] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-fadeIn overflow-y-auto">
        <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-3xl w-full max-w-5xl my-auto max-h-[96vh] sm:max-h-[90vh] flex flex-col shadow-xl overflow-hidden text-slate-900">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex lg:hidden items-center bg-slate-200/70 p-0.5 rounded-xl text-xs font-semibold">
                <button
                  ...
                  className={`px-2 py-1 rounded-lg flex items-center gap-1 transition-all cursor-pointer ${mobileTab === "editor" ? "bg-white text-purple-700 shadow-xs" : "text-slate-600"
                    }`}
                  title="Editor Mode"
                >
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded-lg flex items-center gap-1 transition-all cursor-pointer ${mobileTab === "preview" ? "bg-white text-purple-700 shadow-xs" : "text-slate-600"
                    }`}
                  title="Preview Mode"
                >
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
            ...
            <div className={`lg:col-span-7 p-4 sm:p-6 overflow-y-auto space-y-5 border-r border-slate-200/80 ${mobileTab === "editor" ? "block" : "hidden lg:block"
              }`}>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="relative">
                          ...
                          {/* Smart Account Briefing Popover */}
                          {showAiAccountPicker && selectedAccountIds.length > 1 && (
                            <div className="absolute top-full left-0 mt-1.5 z-[100] w-64 bg-white border border-purple-200 rounded-2xl shadow-xl p-2.5 space-y-1.5 animate-fadeIn">
                              <div className="flex items-center justify-between border-b border-purple-100 pb-1.5 px-1">
                                <span className="text-[10.5px] font-extrabold text-purple-900 font-['Outfit']">
                                  Pilih Briefing Akun Target:
                                </span>
                                <button
                                  type="button"
                                  onClick={() => setShowAiAccountPicker(false)}
                                ...
                                {availableAccounts
                                  ...
                                  .map((acc) => (
                                    <button
                                      key={acc.id}
                                      type="button"
                                      onClick={() => handleGenerateCaptionFromAi(acc.id)}
                                      className="w-full flex items-center gap-2 p-1.5 rounded-xl hover:bg-purple-50 text-left transition-colors cursor-pointer group"
                                    >
                                      <img
                                        src={acc.avatar_url || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150"}
                                        alt={acc.username}
                                      ...
                                      <div className="flex-1 min-w-0">
                                        <p className="text-[11px] font-bold text-slate-800 truncate group-hover:text-purple-700">@{acc.username}</p>
                                        <p className="text-[8.5px] text-purple-600 uppercase font-extrabold">{acc.platform}</p>
                                      </div>
                                    </button>
                                  ))}
                                <button
                                  type="button"
                                  onClick={() => handleGenerateCaptionFromAi("all")}
                                  className="w-full text-center py-1.5 px-2 rounded-xl bg-slate-100 hover:bg-purple-100 text-purple-700 text-[10px] font-bold transition-colors cursor-pointer mt-1"
                                >
                                  🌐 Universal / Murni Visual Gambar
                                </button>
                              </div>
                            </div>
                          )}
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  {(() => {
                    ...
                    return (
                      <span className={`text-[10px] font-bold font-mono px-2 py-0.5 rounded-full ${tagCount > 5 ? "bg-rose-100 text-rose-700" : "bg-purple-100 text-purple-700"
                        }`}>
                        {tagCount} / 5 Hashtags
                      </span>
                    );
                    ...
                    {showBriefPanel && (
                      <div className="flex items-center p-0.5 rounded-lg bg-purple-100/80 text-[10px] font-bold">
                        <button
                          ...
                          className={`px-2 py-0.5 rounded-md transition-all cursor-pointer ${briefViewMode === "preview" ? "bg-white text-purple-900 shadow-2xs" : "text-purple-700 hover:text-purple-900"
                            }`}
                        >
                          👁 Preview
                        </button>
                        <button
                          ...
                          className={`px-2 py-0.5 rounded-md transition-all cursor-pointer ${briefViewMode === "edit" ? "bg-white text-purple-900 shadow-2xs" : "text-purple-700 hover:text-purple-900"
                            }`}
                        >
                          ✏️ Edit Text
                        </button>
              ...
              <div className="p-4 rounded-2xl bg-pink-50/50 border border-pink-200/80 space-y-2 transition-all">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${applyWatermark ? "bg-pink-600 text-white shadow-2xs" : "bg-slate-200 text-slate-600"
                    }`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                        <div>
                          ...
                          <select
                            ...
                            className={`w-full glass-input rounded-xl px-3 py-1.5 text-xs focus:outline-none ${postType !== "video" ? "bg-slate-100 text-slate-400 cursor-not-allowed opacity-60" : "bg-white cursor-pointer"
                              }`}
                          >
                        ...
                        <div className="flex flex-col justify-end">
                          <label className={`flex items-center gap-2 text-xs font-medium ${postType !== "video" ? "text-slate-400 cursor-not-allowed" : "text-slate-700 cursor-pointer"
                            }`}>
                            <input
                              type="checkbox"
                ...
                <div className="flex items-center gap-1">
                  ...
                  {(["1:1", "4:5", "16:9", "9:16"] as const).map(aspect => (
                    <button
                      ...
                      className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer ${previewAspect === aspect ? "bg-purple-600 text-white shadow-xs" : "bg-slate-200/80 text-slate-600 hover:bg-slate-300"
                        }`}
                    >
                      {aspect}
                    </button>
                          ...
                          <div className="absolute bottom-2 inset-x-0 flex items-center justify-center gap-1.5 z-10">
                            {mediaUrls.map((_, idx) => (
                              <button
                                ...
                                className={`w-2 h-2 rounded-full transition-all cursor-pointer ${previewSlideIndex === idx ? "bg-white scale-125 shadow-sm" : "bg-white/50 hover:bg-white/80"
                                  }`}
                              />
                            ))}
                          </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-3xl w-full max-w-3xl max-h-[88vh] flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="px-4 sm:px-6 py-2.5 bg-purple-50/60 border-b border-purple-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="w-full sm:w-48 h-2 rounded-full bg-purple-200/80 overflow-hidden shrink-0">
                  <div
                    className={`h-full transition-all duration-500 rounded-full ${storageInfo.is_overflow ? "bg-rose-500" : storageInfo.percentage > 80 ? "bg-amber-500" : "bg-purple-600"
                      }`}

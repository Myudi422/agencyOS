...
const DELIVERABLE_STATUS_BADGES: Record<string, { label: string; bg: string; text: string; icon: React.ReactNode }> = {
  pending: { label: "Belum Tayang ⏳", bg: "bg-slate-100", text: "text-slate-600", icon: null },
  submitted: { label: "Sudah Tayang ✅", bg: "bg-emerald-100", text: "text-emerald-800", icon: null },
  approved: { label: "Sudah Tayang ✅", bg: "bg-emerald-100", text: "text-emerald-800", icon: null },
  revision_requested: { label: "Belum Tayang ⏳", bg: "bg-slate-100", text: "text-slate-600", icon: null },
  rejected: { label: "Belum Tayang ⏳", bg: "bg-slate-100", text: "text-slate-600", icon: null },
};
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <div className="flex flex-wrap gap-2 shrink-0">
                        ...
                        >
                          ...
                          <span>{d.content_url ? "Edit Link" : "Input Link Postingan"}</span>
                        </button>
                        <button
                  ...
                  {inputUrl.trim() && (
                    <div className="mt-1.5 flex items-center gap-1.5 text-[11px] font-bold">
                      {inputUrl.includes("instagram.com") && (
                        <span className="px-2 py-0.5 rounded-md bg-gradient-to-r from-purple-500 to-pink-500 text-white flex items-center gap-1">
                          📸 Instagram Post / Reel Terdeteksi
                        </span>
                      )}
                      {inputUrl.includes("tiktok.com") && (
                        <span className="px-2 py-0.5 rounded-md bg-black text-white flex items-center gap-1">
                          🎵 TikTok Video Terdeteksi
                        </span>
                      )}
                      {(inputUrl.includes("youtube.com") || inputUrl.includes("youtu.be")) && (
                        <span className="px-2 py-0.5 rounded-md bg-red-600 text-white flex items-center gap-1">
                          ▶️ YouTube Video / Short Terdeteksi
                        </span>
                      )}
                    </div>
                  )}
